                ...
                match listener.accept() {
                    Ok((stream, _)) => {
                        ...
                        let result = active_conns.fetch_update(
                            ...
                            |cur| if cur >= MAX_CONCURRENT_CONNS { None } else { Some(cur + 1) },
                        );
                        if result.is_err() {
                            let mut s = stream;
                            let _ = s.write_all(
...
fn handle_client(mut stream: TcpStream, inf: Arc<Mutex<LlamaInference>>, model_name: String, api_token: &str) {
    ...
    let response = match (method, path.as_str()) {
        ...
        ("GET", "/ready") => json_response(200, &serde_json::json!({
            "status": "ready",
            "model": model_name
        }).to_string()),
        ("GET", "/v1/models") => {
...
fn handle_chat_completion(body: &str, inf: &Arc<Mutex<LlamaInference>>, model_name: &str) -> String {
...
fn parse_http(raw: &str) -> (&str, String, &str, Option<String>) {
    ...
    if lines.is_empty() { return ("GET", "/".into(), "", None); }
    ...
    for line in &lines[1..] {
        ...
        if let Some(rest) = line.strip_prefix("Origin:").or_else(|| line.strip_prefix("origin:")) {
...
fn read_http_request(stream: &mut TcpStream) -> Result<Vec<u8>, String> {
    ...
    while header_end.is_none() {
        ...
        let n = stream.read(&mut tmp).map_err(|e| format!("读取失败: {}", e))?;
        if n == 0 { break; }
    ...
    let header_end = header_end.ok_or_else(|| "请求格式错误，未找到 HTTP 头结束标记".to_string())?;
