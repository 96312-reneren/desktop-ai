...
use egui::{Color32, RichText, Label, ScrollArea, TextEdit, vec2};
use crate::config;
use crate::markdown;
use super::DesktopAI;
                ...
                if is_user {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Min), |ui| {
                        egui::Frame::default()
                            ...
                            .show(ui, |ui| {
                                ui.add(Label::new(
                                    RichText::new(&msg.content).size(font_size).color(Color32::WHITE)
                                ).selectable(true));
                            });
                    });
                } else {
            ...
            if let Some(ref gen) = self.gen {
                if gen.conv_id == self.current_conv.id && !gen.pending_text.is_empty() {
                    ...
                    egui::Frame::default().fill(bg).corner_radius(12)
                        ...
                        .show(ui, |ui| {
                            ui.add(Label::new(
                                RichText::new(&gen.pending_text).size(font_size)
                            ).selectable(true));
