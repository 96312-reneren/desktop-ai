...
impl DesktopAI {
    pub(crate) fn render_settings(&mut self, ctx: &egui::Context, ui: &mut egui::Ui) {
        ScrollArea::vertical().max_height(500.0).show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                if ui
                    .selectable_value(&mut self.config.theme, "dark".into(), "深色")
                    .clicked()
                {
                    apply_theme(ctx, &self.config.theme);
                }
                if ui
                    .selectable_value(&mut self.config.theme, "light".into(), "浅色")
                    .clicked()
                {
                    apply_theme(ctx, &self.config.theme);
                }
            ...
            ui.horizontal(|ui| {
                ui.label("层数:");
                if ui
                    .add(egui::DragValue::new(&mut gl).range(0..=999).speed(1))
                    .changed()
                {
                    gl = gl.max(0);
                }
            ...
            if gl > 0 {
                ui.label(
                    RichText::new(format!("将 {} 层模型卸载到 GPU (需重启模型)", gl))
                        .size(10.0)
                ...
                if !self.gpu_info.is_empty() {
                    ui.label(
                        RichText::new(format!(
                            "检测到 {} GPU, {:.1}GB VRAM",
                            self.gpu_info[0].name, self.gpu_info[0].vram_gb
                        ))
                        .size(10.0)
                        .color(Color32::GRAY),
                    );
            ...
            ui.add(
                TextEdit::multiline(&mut self.config.system_prompt)
                    .desired_rows(2)
            ...
            ui.checkbox(
                &mut self.config.api_enabled,
                "启用本地 API 服务 (OpenAI 兼容)",
            );
            if self.config.api_enabled {
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .add_sized(vec2(80.0, 20.0), TextEdit::singleline(&mut port_str))
                        .changed()
                    {
                        if let Ok(p) = port_str.parse() {
                            self.config.api_port = p;
                        }
                    }
                });
                ui.label(
                ...
                ui.label(
                    RichText::new("支持 POST JSON, 兼容 OpenAI chat completions 格式")
                        .size(10.0)
                        .color(Color32::GRAY),
                );
            }
            ...
            ui.label(
                RichText::new("搜索按钮在左侧边栏底部")
                    .size(10.0)
                    .color(Color32::GRAY),
            );
            ui.checkbox(&mut self.config.kb_enabled, "启用本地知识库 (RAG)");
            ui.label(
                RichText::new("加载模型后可用。自动检索相关片段注入对话。")
                    .size(10.0)
                    .color(Color32::GRAY),
            );
            ui.add_space(8.0);
            ...
            if let Some(ref sel) = self.config.selected_model_id {
                if let Some(info) = find_model(&self.config.model_catalog, sel) {
                    ui.label(
                        RichText::new(format!("当前模型: {}", info.name))
                            .size(12.0)
            ...
            let downloaded_list: Vec<(String, String, f64)> = self
                .config
                .model_catalog
                .iter()
                .filter_map(|m| {
            ...
            if downloaded_list.is_empty() {
                ...
                for (id, name, size_mb) in &downloaded_list {
                    ui.horizontal(|ui| {
                        ui.label(
                            RichText::new(format!("{}  ({:.0} MB)", name, size_mb)).size(11.0),
                        );
