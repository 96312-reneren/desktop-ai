...
impl EmbeddingEngine {
    pub fn load(model_path: &str, n_ctx: u32, n_threads: u32) -> Result<Self, String> {
        ...
        let dim = unsafe { ffi::n_embd(model) } as usize;
        if dim <= 0 { unsafe { ffi::free_model(model); } return Err("invalid embedding dimension".into()); }

        let ctx = unsafe { ffi::new_embedding_context(model, n_ctx, n_threads) };
        if ctx.is_null() { unsafe { ffi::free_model(model); } return Err("failed to create embedding context".into()); }
    ...
    pub fn embed(&self, text: &str) -> Vec<f32> {
        unsafe {
            ...
            if tokens.is_empty() { return vec![0.0; self.dim]; }

            for chunk in tokens.chunks(512) {
                for &t in chunk { ffi::decode(self.ctx, t); }
            }

            let n_tokens = tokens.len() as i32;
            if n_tokens == 0 { return vec![0.0; self.dim]; }
    ...
    fn unload(&mut self) {
        if !self.ctx.is_null() {
            unsafe { ffi::free_embd_context(self.ctx); }
